...
namespace green::gpu {
  ...
  public:
    ...
    void read_integrals(size_t k1, size_t k2){
      // !NOTE: turning off legacy reads without changing any other dependent codes for now
    ...
    void read_entire(std::complex<type>* Vk1k2_Qij, int intranode_rank, int processes_per_node) {
      ...
      std::runtime_error("cuda_low_cpu_memory=False temporarily disabled");
    }
    ...
    void symmetrize(std::complex<double>* Vk1k2_Qij, tensor<prec, 3>& V, const int k1, const int k2) {
      std::runtime_error("Reading integrals `as_a_whole' is not supported yet!");
    }
